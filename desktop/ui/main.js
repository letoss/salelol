...
async function connect(credentials){
  ...
  session={...credentials,ownerToken:joined.ownerToken};localStorage.setItem(STORAGE_KEY,JSON.stringify({name:riotId(session),...session}));setConnected(true);setStatus("connected","Conectado","Buscando el cliente local de League of Legends…");await poll();pollTimer=setInterval(poll,20_000);
}

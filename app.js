...
function reportMap(report){
  ...
  return `<div class="report-map"><svg viewBox="0 0 150 150" role="img" aria-label="Recorrido de los jugadores sobre el mapa de la Grieta del Invocador"><rect width="150" height="150" fill="#071a20"/><image href="https://ddragon.leagueoflegends.com/cdn/15.15.1/img/map/map11.png" x="0" y="0" width="150" height="150" preserveAspectRatio="xMidYMid slice"/><rect width="150" height="150" fill="#010a13" opacity=".18"/>${paths}</svg><div class="report-map-legend">${legend}</div><small>Posiciones aproximadas por minuto</small></div>`;
}
function reportPlayerCard(player,awards){
  const badges=[
    awards?.manco===player.riotId?{label:"El verdadero manco",description:"Peor puntaje general: considera KDA, tiempo muerto, farmeo, visión, objetivos y daño."}:null,
    awards?.afkFarming===player.riotId?{label:"AFK farming",description:"Más farmeo en relación con su participación en kills y objetivos."}:null,
    awards?.gray===player.riotId?{label:"Me gusta la pantalla gris",description:"Mayor tiempo total muerto entre los jugadores registrados."}:null,
    awards?.carry===player.riotId?{label:"Carrito oficial",description:"Mayor aporte combinado de daño y participación en kills."}:null,
  ].filter(Boolean);
  const dead=`${Math.floor(Number(player.timeDeadSeconds||0)/60)}:${String(Number(player.timeDeadSeconds||0)%60).padStart(2,"0")}`;
  return `<article class="report-player"><header><i style="--player-color:${escapeHtml(player.color)}"></i><div><strong>${escapeHtml(compactRiotId(player.riotId))}</strong><small>${escapeHtml(player.championName||"")}</small></div><b>${Number(player.kills)||0}/${Number(player.deaths)||0}/${Number(player.assists)||0}</b></header><div class="report-stats"><span><small>CS/MIN</small><b>${Number(player.csPerMinute||0).toFixed(1)}</b></span><span><small>CS 10 / 15</small><b>${Number(player.csAt10)||0} / ${Number(player.csAt15)||0}</b></span><span><small>VISIÓN/MIN</small><b>${Number(player.visionPerMinute||0).toFixed(2)}</b></span><span title="${escapeHtml(deadTimeLabel(Number(player.timeDeadSeconds)||0))}"><small>TIEMPO MUERTO</small><b>${dead}</b></span><span><small>OBJETIVOS</small><b>${Number(player.objectiveParticipation?.percent)||0}%</b></span><span><small>ROAMING</small><b>${Number(player.roamingScore)||0}</b></span></div><div class="report-badges">${badges.map(badge=>`<em class="has-tooltip" tabindex="0" data-tooltip="${escapeHtml(badge.description)}" aria-label="${escapeHtml(`${badge.label}: ${badge.description}`)}">${escapeHtml(badge.label)}</em>`).join("")}</div>${awards?.gray===player.riotId?`<p>${escapeHtml(deadTimeLabel(Number(player.timeDeadSeconds)||0))}</p>`:""}</article>`;
}
function renderPostGameReport(report){
  const manco=compactRiotId(report.awards?.manco||report.players?.[0]?.riotId),won=report.result==="win";
  const resultClass=won?"victory":"defeat",resultLabel=won?"VICTORIA":"DERROTA";
  const resultIcon=won?"https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/default/content/src/leagueclient/gamemodeassets/classic_sru/img/icon-victory.png":"https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/default/content/src/leagueclient/gamemodeassets/classic_sru/img/icon-defeat.png";
  return `<div class="post-game-report ${resultClass}"><div class="report-result"><img src="${resultIcon}" alt="Emblema oficial de ${resultLabel.toLowerCase()} de League of Legends" /><div><span>RESULTADO DE LA PARTIDA</span><strong>${resultLabel}</strong></div></div><header><div><span>REPORTE FINAL</span><h2>${won?"Terminó la manqueada":`Report ${escapeHtml(manco)}`}</h2></div><small>${Math.max(1,Math.round(Number(report.durationSeconds||0)/60))} min</small></header><div class="report-layout">${reportMap(report)}<div class="report-players">${(report.players||[]).map(player=>reportPlayerCard(player,report.awards)).join("")}</div></div></div>`;
}
...
async function loadLiveGames(){if(!currentInviteCode||document.hidden)return;const result=await remoteStore.fetchLiveGames(currentInviteCode);const players=Array.isArray(result?.players)?result.players:[],report=result?.postGameReport||null;const reportStart=report?new Date(report.gameStart).getTime():0;const hasNewGame=players.some(player=>Number(player.gameStartTime)>reportStart+60_000);const showReport=Boolean(report&&!hasNewGame);const visiblePlayers=showReport?[]:players;captureLiveActions(visiblePlayers);liveGamePlayers=visiblePlayers;liveGameOutcome=showReport?report:null;renderLiveGames();}
